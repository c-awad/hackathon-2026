...
claims = {
    ...
    "notes": (
        ...
        "near-FIFO with working backfill. Because the elastic rule is reactive and this sample "
        "cannot test the return-to-busy case (136 of the jobs it starts early run over 24h), we "
        "also tested two alternatives (ANALYSIS.md case 10). Forecasting each researcher's quota "
        "from two weeks of demand FAILS: demand is bursty, the forecast is 2x too low in 21% of "
        "active weeks, waiting rises 1,316%. A credits pool -- a researcher who is away lends "
        "unused quota for a window of the mean job length (5.1h), their own quota never reduced "
        "-- is inert when gated on requested limits (over-asked 2,057x) but WORKS when gated on "
        "predicted runtime (owner's causal p90): -11% waiting, -21% with the idle timeout, and "
        "lenders blocked for 4.3 researcher-hours in 18 weeks. Its weakness: 26% of borrowed "
        "GPU-hours overran the window, so production needs preemption past the window.\n\n"
